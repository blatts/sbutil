// -*- mode: C++ -*-
// Time-stamp: "2018-03-29 18:01:32 srlab"

/*
  file       CommandLine.cc
  copyright  (c) Sebastian Blatt 2011 -- 2018

*/

#include <sbutil/CommandLine.hh>
#include <sbutil/Representable.hh>
#include <sbutil/StringVector.hh>
...
std::ostream& CommandLine::PrintHelpTexts(std::ostream& out) const {
    ...
    // Calculate length of flag field
    size_t max_len = 0;
    for(auto i = help_texts.begin(); i != help_texts.end(); ++i){
        const size_t id = i->first;
        size_t flag_len = 0;
        size_t flag_multiplicity = 0;
        for(auto j = flag_ids.begin(); j != flag_ids.end(); ++j){
            if(j->second == id){
                flag_len += j->first.size();
                ++flag_multiplicity;
            }
        }
        max_len = std::max(max_len, flag_len +
                           (flag_multiplicity - 1) * flag_sep.size());
    }

    for(auto it = help_texts.begin(); it != help_texts.end(); ++it) {
        ...
        out << "  " << std::setw(max_len) << join(fs, flag_sep)
            << " : " << help_texts.at(id) << "\n";
    }
    return out;
}
                      ...
                      size_t length)
{
    ...
    if(cl.IsFlagDefined("-h")){
        std::cout << program_name << " command line options:\n";
